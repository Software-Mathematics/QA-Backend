...
import com.commons.data.dao.daoImplementation.FeatureDatasetDao;
...
@Service
public class FeatureDatasetServiceImpl implements ServiceI {
    @Autowired
    private FeatureDatasetDao dao;
    ...
    public BaseDtoI create(BaseDtoI baseDtoI) throws AppException, NoSuchMethodException, InstantiationException, IllegalAccessException, InvocationTargetException {
        try {
            ...
            if (baseDtoI.getId() == null) {
                baseDtoI.setId(baseDao.generateSequence(FeatureDataset.SEQUENCE_NAME));
            }
            baseDtoI.setSyncstatus(notSyncedStatus);
            baseDtoI.setStatus(baseDtoI.getStatus().toUpperCase());
            FeatureDataset entity = (FeatureDataset) baseAdapter.dtoToEntity(baseDtoI, new FeatureDataset());
    ...
    public List<BaseDtoI> get(Map<String, Object> map) throws AppException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        try {
            ...
            return (List) baseAdapter.entityToDTO(baseDao.getV2(map, new FeatureDataset()), new FeatureDatasetDTO());
        } catch (Exception e) {
            throw new AppException(e.getMessage());
        }
    }
