...
@Service
public class LoginServiceImplV2 implements ServiceI {

    public static Logger LOG = LoggerFactory.getLogger(LoginServiceImplV2.class);
    @Autowired
...
//    Set<String> restrictedTypes = Set.of("EMAIL", "MOBILE", "USERNAME", "EMAIL||MOBILE");
...
//    );
    ...
    public BaseDtoI create(BaseDtoI baseDtoI) throws Exception {
        ...
        if (entity.getType() == null){
            throw new AppException(errorRepository.getError("E002"));
        }
        if (!LoginConstant.restrictedTypes.contains(entity.getType().toUpperCase())) {
            throw new AppException(errorRepository.getError("E003"));
        }

        if (entity.getEmailid() != null) {
            entity.setEmailid(entity.getEmailid().trim().toUpperCase());
        }
        if (entity.getUsername() != null) {
            entity.setUsername(entity.getUsername().trim().toUpperCase());
        }
        if (entity.getMobileno() != null) {
            entity.setMobileno(entity.getMobileno().trim());
        }
        if (entity.getResourcecode() != null) {
            entity.setResourcecode(entity.getResourcecode().trim().toUpperCase());
        }
        if (entity.getId() == null) {
                List<String> keys = Tokenizer.tokenize(LoginConstant.keyMap.get(entity.getType()), "||");
            List<Object> values = new ArrayList<>();
            for (String key : keys) {
                Field field = entity.getClass().getDeclaredField(key);  // Get field by name
                field.setAccessible(true);  // Allow access to private fields
                values.add(field.get(entity));
            }
            if (isUsernameExistV3(keys, values, entity.getResourcecode(), entity.getType())) {
    ...
    public BaseDtoI login(LoginDTO dto) throws AppException, InvocationTargetException, NoSuchMethodException, IllegalAccessException, InstantiationException {
        if (dto.getUsername() != null && dto.getResourcecode() != null && dto.getPassword() != null && dto.getType() != null) {

            String username = dto.getUsername().trim().toUpperCase();
            String resourceCode = dto.getResourcecode().trim().toUpperCase();
            ...
            throw new AppException(errorRepository.getError("E004"));
        }
    ...
    public boolean isUsernameExistV3(List<String> keyList, List<Object> valueList, String resourceCode, String type) throws AppException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        boolean res = false;
        Map<String, Object> map = new HashMap<>();
        for (int i=0; i < keyList.size(); i++) {
            map = new HashMap<>();
    ...
    public boolean isUsernameExistV2(List<String> keyList, List<Object> valueList, String resourceCode) throws AppException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        Map<String, Object> map = new HashMap<>();
        for (int i=0; i < keyList.size(); i++) {
            map.put(keyList.get(i), valueList.get(i));
        }
    ...
    public boolean isUsernameExist(String email, String mobile, String resourceCode, String roleCode) throws AppException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        boolean res = false;
        List<Login> entityList = (roleCode.equalsIgnoreCase("PA001")) ? dao.findByEmailid(email) : dao.findByEmailidIgnoreCaseOrMobilenoIgnoreCaseAndResourcecodeIgnoreCase(email, mobile, resourceCode);
