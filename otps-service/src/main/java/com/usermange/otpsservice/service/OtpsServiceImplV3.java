...
@Service
public class OtpsServiceImplV3 implements ServiceI {
...
//            publisher.createProducerV2("Otps_UPDATE",entity);
    ...
    public Login getLoginUserV2(List<String> keyList, Object value, String resourceCode, String type) throws AppException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        Login response = null;
        Map<String, Object> map = new HashMap<>();
    ...
    public BaseDtoI confirmV3(String otp, String userid, String msgType, String deviceToken) throws AppException, InvocationTargetException, NoSuchMethodException, IllegalAccessException, InstantiationException, JsonProcessingException {
        Otps otps;
...
//            otps = dao.findByUseridAndOtp(userid.trim(), otp.trim());
            ...
            otpMap.put("userid", userid.trim());
            otpMap.put("otp", otp.trim());
            List<Otps> otpsList = baseDao.get(otpMap, new Otps());
            if (otpsList.isEmpty()) {
                throw new AppException(errorRepository.getError("K006"));
            }
            otps = otpsList.get(otpsList.size() - 1);
...
//            Login login = getLoginUser(otps.getUserid(), otps.getUserid(), otps.getResourcecode());
            List<String> keys = Tokenizer.tokenize(LoginConstant.keyMap.get(msgType.trim().toUpperCase()), "||");
            Login login = getLoginUserV2(keys, otps.getUserid(), otps.getResourcecode(), msgType);
...
//            }
            ...
            if (otps.getExtra_fields() != null && otps.getExtra_fields().get("complaint_no") != null) {
                String complaintNo = otps.getExtra_fields().get("complaint_no").toString();

                Map<String, Object> map = new HashMap<>();
                map.put("code", complaintNo);

                List<Job> jobs = (List<Job>) baseDao.get(map, new Job());

                if (jobs.isEmpty()) {
                    throw new AppException("No records found for the given complaintNo.");
                }

                Job job = jobs.get(jobs.size()-1);
                job.getExtradetail().put("happycode", otp);
                jobDao.save(job);
            }
